...
import { X, Search, Plus, Trash2, CheckCircle2, Loader2, Wrench, Building2, Car, Clock, Boxes, AlertCircle, Check } from 'lucide-react';
...
export default function EditQuoteModal({ quote, onClose, onQuoteUpdated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (plate && plate.trim()) {
      ...
      if (!check.valid) {
        alert(`No se puede guardar la cotización: ${check.conflict?.message || 'Conflicto de placa y modelo'}.\n\nEn el sistema vehicular del Perú cada placa pertenece a un único vehículo físico.`);
        return;
      }
    }
